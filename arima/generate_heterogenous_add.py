...
def arima_custom(hospital, phi1, theta1, phi2, theta2, start_year = 2008, end_year = 2018, avg = 200, d =1, t = 0, mu = 0, sigma = 1):
    ...
    transfused = custom_ARIMA(phi = phi1, theta = theta1, d = d, t = t, mu = mu, sigma= sigma, n = len(res.index)) # simulate time series
    res_transfused = [i[0] + avg for i in transfused]

    transfused = custom_ARIMA(phi = phi2, theta = theta2, d = d, t = t, mu = mu, sigma= sigma, n = len(res.index)) # simulate time series
    res_transfused2 =  [i[0] + avg for i in transfused]

    res_transfused = [x + y for x, y in zip(res_transfused, res_transfused2)]
